  ...
  tables.forEach((table) => {
    ...
    table.columns.forEach((column) => {
      ...
      if (
        ...
        if (referencedTable) {
          constraints.push(
            `.references(() => ${referencedTable.name}.${column.references.columnName})`
          );
        }
      }
    ...
    table.columns.forEach((column) => {
      ...
      if (column.foreignKey && column.references.tableId) {
        ...
        if (referencedTable) {
          ...
          code += `    private ${capitalizeFirstLetter(
            referencedTable.name
          )} ${toCamelCase(referencedTable.name)}Reference;\n\n`;
        }
      } else {
...
export const convertToPrisma = (tables) => {
  ...
  tables.forEach((table) => {
    table.columns.forEach((column) => {
      if (column.foreignKey && column.references.tableId) {
        ...
        if (referencedTable) {
          const relationName = `${table.name}To${capitalizeFirstLetter(
            referencedTable.name
          )}`;
  ...
  tables.forEach((table) => {
    ...
    table.columns.forEach((column) => {
      ...
      if (column.unique && !column.primaryKey) constraints.push("@unique");
      if (column.primaryKey && column.type.toLowerCase() === "int")
        constraints.push("@default(autoincrement())");
    ...
    table.columns.forEach((column) => {
      if (column.foreignKey && column.references.tableId) {
        ...
        if (referencedTable) {
          ...
          code += `  ${toCamelCase(
            referencedTable.name
          )} ${refTableName}? @relation(name: "${relationName}", fields: [${
            column.name
          }], references: [${column.references.columnName}])\n`;
        }
      }
    });
    ...
    if (backReferences && backReferences.length > 0) {
      backReferences.forEach((ref) => {
        ...
        code += `  ${toCamelCase(
          ref.fromTable.name
        )}s ${sourceTableName}[] @relation(name: "${ref.relationName}")\n`;
      });
    }
